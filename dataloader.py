...
class TextDS:
    def __init__(self, path, preprocessor=Preprocessor(), num_terms=10000, batch_size=params.batch_size, split=params.split, window_size=params.window_size, dev='cpu'):
        """
    ...
    def batch_method(self, batch):
        ...
        for sentence_index in range(batch_size):
            ...
            start_index = w
            end_index = len(current_sentence) - w
            increment_size = 1

            for word_index in range(start_index, end_index, increment_size):
                current_word = current_sentence[word_index]
                y_sentence.append(current_word)  # add the current center word to y

                current_context = []
                current_context.append(current_sentence[word_index - w: word_index])  # pre words
                current_context[0].extend(current_sentence[word_index + 1: word_index + 1 + w])  # post words
                x_sentence.append(current_context[0])  # add to x
...
class LabelledTextDS(TextDS):
    ...
    def get_vector_representation(self):
        ...
        vectorizer = CountVectorizer(lowercase=False,
                                     ...
                                     stop_words=None,
                                     max_features=40000,  ## This only keeps the 100 most frequent words as features
